...
%  function varargout = localMaximum(x,minDist,exculdeEqualPoints)
%
%  This function returns the indexes\subscripts of local maximum in
%  the data x, that can be a vector or a matrix of any dimension.
%
%  minDist is the minimum distance between two peaks (local maxima);
%  minDist should be a vector in which each argument corresponds to
%  it's relevant dimension OR a number which is the minimum distance
%  for all dimensions.
%
%  exculdeEqualPoints is a boolean definning either to recognize
%  points with the same value as peaks or not.
%
%      x = [1 2 3 4 4 4 4 4 4 3 3 3 2 1];  
%
%  will the program return all the '4' as peaks or not is defined by
%  the 'exculdeEqualPoints' option (true/false, 0/1)
%
%      localMaximum(x,3)
%
%      ans = 
